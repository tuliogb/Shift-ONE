...
type GlobalVelocidade = {
  placa: string;
  qtd_infracoes: number | null;
  dias_pego: number | null;
  tratativa: number | null;
  datas_tratativa: string[] | null; // json array de strings ISO
};
...
export default function PaginaPlacaVelocidade() {
  ...
  // GLOBAL
  const [global, setGlobal] = useState<GlobalVelocidade | null>(null);
  ...
  useEffect(() => {
    async function carregar() {
      ...
      const detalhes = resumo?.detalhes_por_placa?.[placaNormalizada] ?? [];
      ...
      const { data: glob, error: globErr } = await supabase
        ...
        .select("placa, qtd_infracoes, dias_pego, tratativa, datas_tratativa")
      ...
      if (globErr) {
        ...
        setGlobal({
          placa: glob.placa,
          qtd_infracoes: glob.qtd_infracoes ?? 0,
          dias_pego: glob.dias_pego ?? 0,
  ...
  async function realizarTratativa() {
    try {
      ...
      // precisa existir linha na global pra atualizar
      if (!global) {
        setErro("Essa placa ainda não existe na velocidade_global (sem histórico).");
        return;
      }
      ...
      <section className="mx-auto max-w-5xl px-6 py-10">
        <div className="rounded-3xl border border-white/10 bg-white/10 p-6 shadow-xl backdrop-blur-xl">
          ...
          <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-5">
            <div className="flex flex-wrap items-end justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-white/70">
                  Base: <span className="text-white/90 font-semibold">velocidade_global</span> +{" "}
            ...
            <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-4">
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                <p className="text-xs text-white/60">Infrações (histórico)</p>
                <p className="mt-1 text-lg font-semibold">{global?.qtd_infracoes ?? "-"}</p>
              </div>
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                <p className="text-xs text-white/60">Dias do sistema</p>
                <p className="mt-1 text-lg font-semibold">{diasSistema ?? "-"}</p>
              </div>
                ...
                <div className="mt-1 text-xs text-white/80">
                  {global?.datas_tratativa && global.datas_tratativa.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {global.datas_tratativa.slice().reverse().map((d, idx) => (
                        <span
                          key={`${d}-${idx}`}
          ...
          {carregando ? (
            ...
            <div className="mt-6 text-sm text-white/70">Nenhuma infração encontrada para esta placa.</div>
          ) : (
              ...
              <div className="divide-y divide-white/10">
                {infracoes.map((i, idx) => (
                  <div key={idx} className="grid grid-cols-5 items-center px-4 py-3 text-sm">
                    ...
                    <div className="text-right">{i.vel_media == null ? "-" : `${i.vel_media} km/h`}</div>
                  </div>
                ))}
              </div>
            </div>
